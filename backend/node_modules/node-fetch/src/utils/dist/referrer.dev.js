...
exports.DEFAULT_REFERRER_POLICY = exports.ReferrerPolicy = void 0;
...
function determineRequestsReferrer(request) {
  ...
  var policy = request.referrerPolicy; // 2. Let environment be request's client.
  ...
  var referrerSource = request.referrer; // 4. Let request's referrerURL be the result of stripping referrerSource for use as a referrer.

  var referrerURL = stripURLForUseAsAReferrer(referrerSource); // 5. Let referrerOrigin be the result of stripping referrerSource for use as a referrer, with the
  //    origin-only flag set to true.

  var referrerOrigin = stripURLForUseAsAReferrer(referrerSource, true); // 6. If the result of serializing referrerURL is a string whose length is greater than 4096, set
...
function parseReferrerPolicyFromHeader(headers) {
  ...
  var policyTokens = (headers.get('referrer-policy') || '').split(/[,\s]+/); // 2. Let policy be the empty string.

  var policy = ''; // 3. For each token in policy-tokens, if token is a referrer policy and token is not the empty
